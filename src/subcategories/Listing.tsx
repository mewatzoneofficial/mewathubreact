...
import API_URL, { SUBCATEGORY_TITLE } from "../utils/config";
...
const Listing: React.FC = () => {
  ...
  const fetchRecords = async (pageNumber = 1) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}subcategories?${queryParams.toString()}`);
  ...
  return (
    <div>
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h4 className="fw-semibold">{SUBCATEGORY_TITLE}</h4>
        <div>
          <NavLink to="/subcategories/create" className="btn btn-primary btn-sm me-2">
            <i className="fa-solid fa-plus"></i> Add New {SUBCATEGORY_TITLE}
          </NavLink>
        </div>
      </div>
                  ...
                  results.map((result) => (
                    <tr key={result.ID}>
                      ...
                      <td className="text-right">
                        <Link
                          to={`/subcategories/edit/${result.ID}`}

...
import API_URL, { QUALIFICATION_TITLE } from "../utils/config";
...
const Listing: React.FC = () => {
  ...
  const fetchRecords = async (pageNumber = 1) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}qualifications?${queryParams.toString()}`);
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(`${API_URL}qualifications/${id}`, { method: "DELETE" });
  ...
  return (
    <div>
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h4 className="fw-semibold">{QUALIFICATION_TITLE}</h4>
        <div>
          <NavLink to="/qualifications/create" className="btn btn-primary btn-sm me-2">
            <i className="fa-solid fa-plus"></i> Add New {QUALIFICATION_TITLE}
          </NavLink>
        </div>
      </div>
                  ...
                  results.map((result) => (
                    <tr key={result.ID}>
                      ...
                      <td className="text-right">
                        <Link
                          to={`/qualifications/edit/${result.ID}`}

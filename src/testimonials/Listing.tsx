...
import API_URL, { TESTIMONIAL_TITLE } from "../utils/config";
...
const Listing: React.FC = () => {
  ...
  const fetchRecords = async (pageNumber = 1) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}testimonials?${queryParams.toString()}`);
  ...
  return (
    <div>
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h4 className="fw-semibold">{TESTIMONIAL_TITLE}s</h4>
        <div>
          <NavLink to="/testimonials/create" className="btn btn-primary btn-sm me-2">
            <i className="fa-solid fa-plus"></i> Add New {TESTIMONIAL_TITLE}
          </NavLink>
        </div>
      </div>
                ...
                {results.length === 0 ? (
                  ...
                  results.map((result) => (
                    <tr key={result.ID}>
                      <td>{result.ID}</td>
                      <td>
                        <img src={result.image} alt="Testimonial" />
                      </td>
                      <td>{result.name}</td>
                      ...
                      <td className="text-right">
                        <Link
                          to={`/testimonials/edit/${result.ID}`}

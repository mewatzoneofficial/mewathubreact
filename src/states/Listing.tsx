...
import API_URL, { STATE_TITLE } from "../utils/config";
...
const Listing: React.FC = () => {
  ...
  const fetchRecords = async (pageNumber = 1) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}states?${queryParams.toString()}`);
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(`${API_URL}states/${id}`, { method: "DELETE" });
  ...
  return (
    <div>
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h4 className="fw-semibold">{STATE_TITLE}s</h4>
        <div>
          <NavLink to="/states/create" className="btn btn-primary btn-sm me-2">
            <i className="fa-solid fa-plus"></i> Add New {STATE_TITLE}
          </NavLink>
        </div>
      </div>

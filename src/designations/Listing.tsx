...
import API_URL, { DESIGNATION_TITLE } from "../utils/config";
...
const Listing: React.FC = () => {
  const [results, setResults] = useState<Designation[]>([]);
  ...
  const fetchRecords = async (pageNumber = 1) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}designations?${queryParams.toString()}`);
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(`${API_URL}designations/${id}`, { method: "DELETE" });
  ...
  return (
    <div>
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h4 className="fw-semibold">{DESIGNATION_TITLE}</h4>
        <div>
          <NavLink to="/designations/create" className="btn btn-primary btn-sm me-2">
            <i className="fa-solid fa-plus"></i> Add New {DESIGNATION_TITLE}
          </NavLink>
        </div>
      </div>
                  ...
                  results.map((result) => (
                    <tr key={result.id}>
                      ...
                      <td className="text-right">
                        <Link
                          to={`/designations/edit/${result.id}`}

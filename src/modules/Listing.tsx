...
import API_URL, { MODULE_TITLE } from "../utils/config";
...
const Listing: React.FC = () => {
  ...
  const fetchRecords = async (pageNumber = 1) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}modules?${queryParams.toString()}`);
  ...
  return (
    <div>
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h4 className="fw-semibold">{MODULE_TITLE}s</h4>
        <div>
          <NavLink to="/modules/create" className="btn btn-primary btn-sm me-2">
            <i className="fa-solid fa-plus"></i> Add New {MODULE_TITLE}
          </NavLink>
        </div>
      </div>
                  ...
                  results.map((result) => (
                    <tr key={result.moduleID}>
                      ...
                      <td className="text-right">
                        <Link
                          to={`/modules/edit/${result.moduleID}`}
                        ...
                        <button
                          ...
                          onClick={() => handleDelete(result.moduleID)}
                        >
                          Delete
                        </button>
                      </td>

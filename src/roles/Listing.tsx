...
import API_URL, { ROLE_TITLE } from "../utils/config";
...
const Listing: React.FC = () => {
  ...
  const fetchRecords = async (pageNumber = 1) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}roles?${queryParams.toString()}`);
  ...
  return (
    <div>
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h4 className="fw-semibold">{ROLE_TITLE}</h4>
        <div>
          <NavLink to="/roles/create" className="btn btn-primary btn-sm me-2">
            <i className="fa-solid fa-plus"></i> Add New {ROLE_TITLE}
          </NavLink>
        </div>
      </div>
            ...
            <table className="table table-bordered table-hover align-middle">
              ...
              <tbody>
                {results.length === 0 ? (
                  ...
                  results.map((result) => (
                    <tr key={result.roleID}>
                      <td>{result.roleID}</td>
                      <td>{result.name}</td>
                      <td>{formatDMY(result.created_at)}</td>
                      <td className="text-right">
                        <Link
                          to={`/roles/edit/${result.roleID}`}
                        ...
                        <button
                          ...
                          onClick={() => handleDelete(result.roleID)}
                        >
                          Delete
                        </button>
                      </td>

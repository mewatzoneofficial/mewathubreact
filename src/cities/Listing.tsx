...
import API_URL, { CITY_TITLE } from "../utils/config";
...
const Listing: React.FC = () => {
  ...
  const fetchRecords = async (pageNumber = 1) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}cities?${queryParams.toString()}`);
  ...
  return (
    <div>
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h4 className="fw-semibold">{CITY_TITLE}s</h4>
        <div>
          <NavLink to="/cities/create" className="btn btn-primary btn-sm me-2">
            <i className="fa-solid fa-plus"></i> Add New {CITY_TITLE}
          </NavLink>
        </div>
      </div>

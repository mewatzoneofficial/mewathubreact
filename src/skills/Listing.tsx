...
import API_URL, { SKILL_TITLE } from "../utils/config";
...
const Listing: React.FC = () => {
  ...
  const fetchRecords = async (pageNumber = 1) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}skills?${queryParams.toString()}`);
  ...
  return (
    <div>
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h4 className="fw-semibold">{SKILL_TITLE}</h4>
        <div>
          <NavLink to="/skills/create" className="btn btn-primary btn-sm me-2">
            <i className="fa-solid fa-plus"></i> Add New {SKILL_TITLE}
          </NavLink>
        </div>
      </div>

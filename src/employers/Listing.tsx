...
import API_URL, { EMPLOYER_TITLE } from "../utils/config";
...
const Listing: React.FC = () => {
  ...
  const fetchUsers = async (pageNumber = 1) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}employers?${queryParams.toString()}`);
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(`${API_URL}employers/${id}`, { method: "DELETE" });
  ...
  if (loading) return <div style={{ display: 'flex', justifyContent: 'center' }}><CustomLoading /> </div>;
  
  return (
    <div>
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h4 className="fw-semibold">{EMPLOYER_TITLE}s</h4>
        <div>
          <NavLink to="/employers/create" className="btn btn-primary btn-sm me-2">
            <i className="fa-solid fa-plus"></i> Add New {EMPLOYER_TITLE}
          </NavLink>
        </div>
      </div>
            ...
            <table className="table table-bordered table-hover align-middle">
              ...
              <tbody>
                {results.length === 0 ? (
                  ...
                  results.map((result) => (
                    <tr key={result.employerID}>
                      <td>{result.employerID}</td>
                      <td>{result.name}</td>
                      <td>{result.email}</td>
                      <td>{result.mobile}</td>
                      ...
                      <td className="text-right">
                        <Link
                          to={`/employers/edit/${result.employerID}`}
                        ...
                        <button
                          ...
                          onClick={() => handleDelete(result.employerID)}
                        >
                          Delete
                        </button>
                      </td>

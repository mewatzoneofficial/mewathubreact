...
import API_URL, { VIEWLOG_TITLE } from "../utils/config";
...
const Listing: React.FC = () => {
  const [results, setResults] = useState<ViewLog[]>([]);
  ...
  const fetchRecords = async (pageNumber = 1) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}view-logs?${queryParams.toString()}`);
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(`${API_URL}view-logs/${id}`, { method: "DELETE" });
  ...
  return (
    <div>
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h4 className="fw-semibold">{VIEWLOG_TITLE}</h4>
        <div>
          <NavLink to="/view-logs/create" className="btn btn-primary btn-sm me-2">
            <i className="fa-solid fa-plus"></i> Add New {VIEWLOG_TITLE}
          </NavLink>
        </div>
      </div>
                ...
                {results.length === 0 ? (
                  ...
                  results.map((result) => (
                    <tr key={result.id}>
                      ...
                      <td>{result.name}</td>
                      <td>{result.module}</td>
                      <td>{result.module_type}</td>
                      <td>{formatDMY(result.created_at)}</td>
                      <td className="text-right">
                        <Link
                          to={`/view-logs/edit/${result.id}`}
